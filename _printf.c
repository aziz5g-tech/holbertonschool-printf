...
int _printf(const char *format, ...)
{
        ...
        for (i = 0; format[i]; i++)
        {
                if (format[i] == '%')
                {
                        ...
                        /* If no valid specifier after flags, print '%' only,
                         * then reprocess the next char in the next loop. */
                        if (format[i] == '\0' ||
                              ...
                              format[i] == 'X'))
                        {
                                ...
                                count += 1;
                                i = j - 1; /* next loop will handle format[j] */
                                continue;
                        }

                        /* Avoid unused-variable warning for j when not used. */
                        (void)j;
